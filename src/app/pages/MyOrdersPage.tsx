...
export function MyOrdersPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // üîÑ Silent background polling every 3 seconds for faster real-time updates
      const interval = setInterval(() => {
        console.log('üîÑ [MyOrdersPage] Background polling for status updates...');
  ...
  const fetchOrders = async (isBackgroundFetch: boolean = false) => {
    try {
      ...
      if (isBackgroundFetch) {
        console.log('üîÑ [MyOrdersPage] Silent background fetch - no loading spinner');
      } else {
        console.log('üì¶ [MyOrdersPage] Initial fetch - showing loading spinner');
      }
      
      console.log('üì¶ [MyOrdersPage] Fetching orders for user:', user?.email);
      console.log('üì¶ [MyOrdersPage] User ID:', user?.id);
      console.log('üì¶ [MyOrdersPage] Access Token:', accessToken ? 'Present' : 'Missing');
      
      let token = accessToken;
      
      // üîê For real Backend users (not mock users), refresh the session to get a fresh token
      if (token && !token.startsWith('mock-token-')) {
        try {
          console.log('îÑ [MyOrdersPage] Refreshing Backend session for fresh token...');
          const { data: { session }, error: sessionError } = await authClient.auth.getSession();
          
          if (session?.access_token) {
            token = session.access_token;
            console.log('Ö [MyOrdersPage] Got fresh token from session');
          } else {
      ...
      if (token) {
        try {
          if (!isBackgroundFetch) {
            console.log('üì° [MyOrdersPage] Fetching from backend...');
            console.log('üì° [MyOrdersPage] Backend URL:', `https://${projectId}.authClient.co/functions/v1/make-server-a67f0635/orders/my-orders`);
            console.log('üì° [MyOrdersPage] User token type:', token.startsWith('mock-token-') ? 'DEMO USER (mock-token)' : 'REAL USER (Backend token)');
          }
          ...
          // üî• Add cache-busting timestamp to prevent browser from returning stale cached data
          const cacheBuster = `?t=${Date.now()}`;
          
          const response = await fetch(
            `https://${projectId}.authClient.co/functions/v1/make-server-a67f0635/orders/my-orders${cacheBuster}`,
            {
          ...
          if (response.ok) {
            ...
            if (isBackgroundFetch) {
              console.log(`üîÑ [Background] Loaded ${backendOrders.length} orders - updating silently`);
            } else {
              console.log(`‚úÖ [MyOrdersPage] Loaded ${backendOrders.length} orders from backend`);
            }
            ...
            backendOrders.forEach((order, idx) => {
              ...
              console.log(`îç [Order ${idx + 1}/${backendOrders.length}] ID:`, order.id);
              console.log(`   ìã Order Number:`, order.order_number);
              console.log(`   ì Status:`, order.status);
              console.log(`   ì¶ Tracking:`, order.tracking_number || 'NOT SET');
      ...
      for (const localOrder of parsedLocalOrders) {
        if (!backendOrderIds.has(localOrder.id)) {
          console.log(`üìã [MyOrdersPage] Adding localStorage-only order: ${localOrder.order_number}`);
      ...
      if (isBackgroundFetch) {
        ...
        console.log('Ø [MyOrdersPage] Orders loaded and merged from backend + localStorage');
      }
    } catch (error) {
  ...
  const downloadCompletedFile = async (orderId: string) => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.authClient.co/functions/v1/make-server-a67f0635/orders/${orderId}/download-completed-file`,
        {
          headers: buildHeaders(token)
        }
      );
  ...
  const getOrderTrackingSteps = (order: Order) => {
    ...
    if (order.status.toLowerCase() === 'cancelled') {
      console.log('üî¥ [Tracking Steps] Order is cancelled');
      return [
    ...
    steps.forEach((step, idx) => {
      console.log(`  Step ${idx + 1}: ${step.status} - ${step.completed ? '‚úÖ Completed' : '‚è≥ Pending'}`);
    });
