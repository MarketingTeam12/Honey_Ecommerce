...
export function CartPage() {
  ...
  const loadCouponsFromStorage = () => {
    try {
      ...
      if (storedCoupons) {
        ...
        console.log('📦 [CartPage] ✅ State updated!');
      } else {
        console.log('⚠️ [CartPage] No coupons found in localStorage with key:', STORAGE_KEY);
        console.log('📦 [CartPage] All localStorage keys:', Object.keys(localStorage));
        setCoupons([]);
      }
    } catch (error) {
      console.error('❌ [CartPage] Error loading coupons:', error);
      setCoupons([]);
    }
  };
  ...
  const handleApplyCoupon = (code: string) => {
    ...
    if (!coupon) {
      console.error('❌ [CartPage] Coupon not found in available coupons');
    ...
    console.log('✅ [CartPage] Coupon found:', coupon);

    if (!coupon.isActive) {
      console.error('❌ [CartPage] Coupon is not active');
    ...
    if (coupon.minOrderValue && subtotal < coupon.minOrderValue) {
      console.error('❌ [CartPage] Order value too low. Subtotal:', subtotal, 'Min:', coupon.minOrderValue);
    ...
    if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) {
      console.error('❌ [CartPage] Usage limit reached. Used:', coupon.usedCount, 'Limit:', coupon.usageLimit);
    ...
    if (coupon.validFrom) {
      ...
      if (today < validFrom) {
        console.error('❌ [CartPage] Coupon not yet valid. Valid from:', validFrom);
    ...
    if (coupon.validUntil) {
      ...
      if (today > validUntil) {
        console.error('❌ [CartPage] Coupon expired. Valid until:', validUntil);
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-4">
                          ...
                          <div className="text-right">
                            <p className="text-sm text-gray-500">
                              {formatPrice(item.basePrice)} × {item.pageCount}
                            </p>
